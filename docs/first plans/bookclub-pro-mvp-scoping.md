...
**Uwaga:** Konta, subskrypcje i płatności są zapewnione z zewnątrz (external auth/billing).
...
| [x] Dołączanie do klubu | Członkowie dołączają przez link (bez rejestracji w MVP) | 3h |
...
1. **Role użytkowników** → Wszyscy członkowie mają równe prawa w MVP
...
|-----------|--------------|--------------|
| **Frontend** | Next.js + Tailwind | Szybki start, gotowe komponenty |
| **Backend** | Next.js API Routes | Proste, bez osobnego serwera |
| **Database** | Supabase (PostgreSQL) | Managed, darmowy tier, gotowa auth |
| **Auth** | Supabase Auth (external) | Integracja z external auth |
| **Hosting** | Vercel | Darmowy tier dla MVP |
