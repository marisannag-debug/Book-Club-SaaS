...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  try {
    const { id } = req.query
    if (req.method === 'GET') {
      const result = await db.query('SELECT id, name, description, organizer_id AS "organizerId", invite_code AS "inviteCode" FROM clubs WHERE id = $1', [id])
      if (result.rowCount === 0) return res.status(404).json({ error: 'Not Found' })
      return res.status(200).json({ club: result.rows[0] })
    }
